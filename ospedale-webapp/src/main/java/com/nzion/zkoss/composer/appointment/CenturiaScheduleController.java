...
public class CenturiaScheduleController extends OspedaleAutowirableComposer {
	...
	public CenturiaScheduleController() {
	...
	public CenturiaScheduleController(String mode) {
		this();
		this.mode = mode;
	...
	public void doAfterCompose(Component component) throws Exception {
		...
		setLocation((Location) Sessions.getCurrent().getAttribute("_location"));
	...
	private void cancelInvoiceLineItem(Invoice invoice, InvoiceItem ii) {
		Money price = invoice.getTotalAmount();
	...
	public void generateRescheduleInvoiceForRCM(Schedule currentSchedule) {
		...
		if (UtilValidator.isEmpty(invoices) && billingDisplayConfig != null) {
			...
			if (billingDisplayConfig.getRegistrationFee() != null) {
				...
				if (invoice.getTotalAmount() != null && invoice.getTotalAmount().getAmount() != null)
					invoice.setTotalAmount(new Money(invoice.getTotalAmount().getAmount().add(billingDisplayConfig.getRegistrationFee()), convertTo()));
				else
					invoice.setTotalAmount(new Money(billingDisplayConfig.getRegistrationFee(), convertTo()));
			}
		}
		RCMVisitType rcmVisitType = null;
		...
		if (schedulingPreference != null && "Y".equals(schedulingPreference.getShowConvenienceFee())) {
			...
			if (invoice.getTotalAmount() != null && invoice.getTotalAmount().getAmount() != null)
				invoice.setTotalAmount(new Money(invoice.getTotalAmount().getAmount().add(consultationCharges), convertTo()));
			else
				invoice.setTotalAmount(new Money(consultationCharges, convertTo()));
			...
			if (invoice.getTotalAmount() != null && invoice.getTotalAmount().getAmount() != null)
				invoice.setTotalAmount(new Money(invoice.getTotalAmount().getAmount().add(convenienceFee), convertTo()));
			else
				invoice.setTotalAmount(new Money(convenienceFee, convertTo()));
		} else {
			itemId = (System.currentTimeMillis() + currentSchedule.getId());
			String invoiceItemDescription = InvoiceType.OPD_CONSULTATION.getDescription() + " - " + currentSchedule.getVisitType().getName();
			...
			if (invoice.getTotalAmount() != null && invoice.getTotalAmount().getAmount() != null)
				invoice.setTotalAmount(new Money(invoice.getTotalAmount().getAmount().add(totalCosultFeeConvFee), convertTo()));
			else
				invoice.setTotalAmount(new Money(totalCosultFeeConvFee, convertTo()));
		}
	...
	public void openWaitingListWindow() {
		...
		wlModalWindow.addEventListener("onDetach", new EventListener() {
			public void onEvent(Event event) throws Exception {
				populateWaitingListLabel();
			}
		});
	...
	public void generateInvoice(Schedule currentSchedule) {
		...
		invoice.addInvoiceItem(consultationItem);
		if (invoice.getTotalAmount() != null && invoice.getTotalAmount().getAmount() != null)
			invoice.setTotalAmount(new Money(invoice.getTotalAmount().getAmount().add(amount), convertTo()));
		else
			invoice.setTotalAmount(new Money(amount, convertTo()));
